...
from MLBClasses import Player, ClassicLineup
...
def LineupOptimizerRecurse(playerList, CurrLineup, BestLineup, currSize, size=10):
        ...
        if currSize == size:
            ...
            for player in playerList:
                if (CurrLineup.total + float(player.salary)) <= 50000:
                    if currSize == 0:
                    ...
                    if player.pos == "P":
                        if CurrLineup.P[0] == "EMPTY":
                            ...
                            BestLineup = LineupOptimizerRecurse([p for p in playerList if not p.name == player.name], CurrLineup, BestLineup, currSize+1, size)
...
if __name__ == '__main__':
    ...
    with open('DKSalaries.csv') as csv_file:
        ...
        for row in csv_reader:
            if line_count > 0:
                #row[2]=name, row[4]=position row[5]=salary row[8]=avg points per game
                temp = Player(row[2],row[4],row[5],row[8])
                if row[4] == "P" and row[0] == "SP":
                    temp.setType("P")
